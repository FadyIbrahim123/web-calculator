...
- Q: What happens when the user selects a history entry to "reuse" it? → A: The entry's result becomes the new current display value, as if the user had just typed that number, ready to be used as the start of a new calculation. It does not restore the original operator/second-operand and does not auto-append to a calculation already in progress.
...
4. **Given** the user performs more calculations than the history's retention limit, **When** the user opens the history view, **Then** only the most recent calculations (up to the limit) are shown and the oldest entries have been dropped silently, with no error.
...
### User Story 2 - Reuse a past result (Priority: P2)

A user sees a result they computed a moment ago and wants to use it as the starting point for a new calculation, without retyping it.
...
**Independent Test**: Can be fully tested by completing a calculation, selecting its history entry, and confirming the calculator's current display becomes that entry's result, ready for further input.

**Acceptance Scenarios**:

1. **Given** the history contains an entry `12 + 7 = 19`, **When** the user selects that entry, **Then** the calculator's display shows `19` as the current entry, with no pending operator, as if the user had just typed `19`.
2. **Given** the user has a calculation already in progress (e.g., they typed `45` and pressed `+`), **When** the user selects a history entry showing `19`, **Then** the in-progress calculation is discarded and the display starts fresh showing `19`.
3. **Given** the user selects a history entry and its value becomes the current display, **When** the user then presses an operator, enters a second number, and presses equals, **Then** the calculation proceeds normally and (if completed) is itself added as a new history entry.
...
- On a narrow screen, the history is not shown expanded by default; it is reached via an explicit toggle so it never competes with the keypad for space.
...
- **FR-004**: The calculator MUST retain only a bounded number of the most recent history entries (a "short" list); once the limit is reached, adding a new entry MUST silently drop the oldest entry.
- **FR-005**: The calculator MUST let the user select any history entry to reuse it; doing so sets the entry's result as the current display value, discarding any calculation currently in progress, without altering the history list itself.
...
- **FR-008**: The history view MUST be presented so that, at all supported screen widths including small phones, no existing calculator control (digits, operators, percent, equals, clear-entry, clear-all, delete-last-digit) becomes overlapped, clipped, or unreachable; on narrow screens the history MUST be reachable via an explicit show/hide control rather than being permanently expanded.
...
- **History List**: The bounded, ordered collection of History Entries currently retained, most recent first, capped at a short maximum count with oldest entries evicted first.
...
- History retention limit is treated as a small, fixed count (e.g., in the range of 10-20 most recent entries) sufficient to feel like a "short" list per the request; the exact number is an implementation detail left to the planning phase as long as it stays small and bounded.
- "Reuse" means loading the entry's result as a fresh starting value (see Clarifications) rather than replaying the full original expression for editing; this matches common calculator app behavior and keeps the interaction to a single step.
...
- The history view is a supplementary UI element (e.g., a panel or list reachable via a toggle) rather than a replacement for the main display; it does not need its own dedicated full-screen mode.
