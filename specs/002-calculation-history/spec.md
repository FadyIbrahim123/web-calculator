...
- **FR-008**: The history view MUST be presented so that, at all supported screen widths including small phones, no existing calculator control (digits, operators, percent, equals, clear-entry, clear-all, delete-last-digit) becomes overlapped, clipped, or unreachable. On narrow screens (down to 320px) the history MUST be a hidden-by-default overlay/drawer sitting behind the keypad, reachable via an explicit show/hide toggle, such that opening or closing it never resizes, reflows, or repositions the keypad.
